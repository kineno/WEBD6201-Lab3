...
(function () {
        ...
        if (sessionStorage.getItem("user")) {
            ...
            if($("#task-list").length === 0){
                ...
                <li class="nav-item" id="task-list">
                    <a class="nav-link" data="task-list"><i class="fas fa-home"></i>Task List</a>
                </li>
                `)
    ...
    function DisplaysTasks(){
        ...
        for(let t of tasks){
            ...
            data += `
                <li class="list-group-item" id="task">
                    <span id="taskText">${t.val}</span>
                    <span class="float-end">
                        <button id="e${i}" class="btn btn-outline-primary btn-sm editButton"><i class="fas fa-edit"></i>
                        <button id="d${i}" class="btn btn-outline-danger btn-sm deleteButton"><i class="fas fa-trash-alt"></i></button>
                    </span>
        ...
        for(let t of tasks){
            $(`#e${i}`).on("click", function(){
                let pormpt = window.prompt("Enter task new text", t.val)
                console.log("herherherherherh ", pormpt)
                tasks[t.id] = {id: t.id, val: pormpt }
                console.log("here again ",tasks)
    ...
    function AddNewTask(){
        $('#newTaskButton').on('click', function(){
            const text = $('#taskTextInput').val();

            if(text === ""){
                alert("Please fill the task with some values");
            }else{
                let tasks = localStorage.getItem('tasks');

                if(!tasks){
                  tasks = []  
                  tasks.push({id: tasks.length, val:text})
                }else{
                    tasks = JSON.parse(tasks)
                    tasks.push({id: tasks.length, val: text})
                }
